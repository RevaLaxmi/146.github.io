...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const sentences = [
    "How many rats in this world?",
    "Around seven billion in the world...",
    "BUT YOU!?...",
    "you’re small but your heart isn’t 🫶 don’t ever be ashamed of yourself, that’s your parents job "
  ];
  let sentenceIndex = 0;
  let textIndex = 0;
      ...
      if (textIndex < text.length) {
        ...
        if (sentenceIndex === sentences.length) {
          setTimeout(function() {
            ...
            imageElement.src = 'rat.png'; // Replace 'path/to/your/image.jpg' with the path to your image
            imageContainer.appendChild(imageElement); // Append the image to the image container
          }, 2000); // Delay before loading the image (milliseconds)
        } else {
          setTimeout(typeNextSentence, 700); // Delay before typing the next sentence
        }
      }
    }, 60); // Adjust typing speed (milliseconds)
  }
